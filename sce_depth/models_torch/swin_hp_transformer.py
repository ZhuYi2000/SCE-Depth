...
from sce_depth.data.segmentation.data_spec import DataSpec
from sce_depth.models_torch import hp_shifting
from sce_depth.models_torch.hp_windowing import window_partition, window_reverse, get_nest_win_idcs
import numpy as np
from pathlib import Path

SCE_DEPTH_ROOT = Path(__file__).resolve().parents[1]


def _runtime_assets_path(path: str) -> str:
    return str(SCE_DEPTH_ROOT / "runtime_assets" / path)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.use_v2_norm_placement:
            x = self.norm1(x)
            # print("Enter: x=self.norm1(x)")
        ###########################################################################
        # debug
        
        # print(self.use_v2_norm_placement, "use_v2_norm_placement")  # True
        ...
        if self.use_v2_norm_placement:
            ...
            # print("Enter Not use_v2_norm_placement")
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        """
        x: B, N, dim
        Expands channels and rearranges tokens for 2x spatial upsampling.
        Effective reshape: (W/32 x H/32 x 16C) -> (W/16 x H/16 x 4C).
        """
        x = self.expand(x)
        B, N, C = x.shape
...
class EdgeAttention(nn.Module):
    ...
    def __init__(self):
        ...
        self.edge_conv = nn.Conv2d(1, 3, kernel_size=1)
        
    def forward(self, img_2d, sobel_2d):
        """
        :
            img_2d:   [B, 3, 128, 128]
            sobel_2d:    [B, 1, 128, 128]
        :
            fused_2d:  [B, 3, 128, 128]
        """
        combined = torch.cat([img_2d, sobel_2d], dim=1)  # [B,4,128,128]
        
        attn_map = self.fusion_conv(combined)  # [B,1,128,128]
        
        edge_mapped = self.edge_conv(sobel_2d)  # [B,3,128,128]
        
        fused_2d = img_2d * (1 - attn_map) + edge_mapped * attn_map
        
        return fused_2d


class EdgeAttention_2(nn.Module):
    """2D"""
    def __init__(self):
        super().__init__()
        self.fusion_conv = nn.Sequential(
        ...
        self.edge_conv = nn.Conv2d(1, 1, kernel_size=1)
...
class EdgeAttention_3(nn.Module):
    """2D"""
    def __init__(self):
        super().__init__()
        self.fusion_conv = nn.Sequential(
    ...
    def forward(self, img_2d, sobel_2d):
        ...
        edge_mapped = self.edge_conv(sobel_2d)  # [B,1,128,128]
        
        fused_2d = img_2d + edge_mapped * attn_map
        
        return fused_2d


class EdgeAttention_4(nn.Module):
    """2D"""
    def __init__(self):
        super().__init__()
        self.fusion_conv = nn.Sequential(
...
class SwinHPSobelTransformerSys(nn.Module):
    r"""Swin Transformer A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer
        using Shifted Windows` - https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, config: SwinHPTransformerConfig, data_spec: DataSpec, **kwargs):
        ...
        self.indices_1d_to_2d = torch.from_numpy(np.load(_runtime_assets_path("hp16384_1d2d_index.npy")))
        self.indices_2d_to_1d = torch.from_numpy(np.load(_runtime_assets_path("hp16384_2d1d_index.npy")))

        self.edge_attention = nn.ModuleList([
            EdgeAttention() for _ in range(8)
        ])
    ...
    def torch_1d_to_2d(self, tensor_1d):
        """
        # Convert a 1D tensor to a 2D tensor using precomputed indices.
        # indices = torch.from_numpy(np.load(_runtime_assets_path("hp16384_1d2d_index.npy")))
        """
        if len(tensor_1d.shape) == 1:
            tensor_1d = tensor_1d.unsqueeze(0)
        bs = tensor_1d.shape[0]
        # print("tensor_1d.shape", tensor_1d.shape)
        return tensor_1d[:, self.indices_1d_to_2d].view(bs, 128, 128)


    def torch_2d_to_1d(self, tensor_2d):
        """
        Convert a 2D tensor back to a 1D tensor using precomputed indices.
        # inverse_indices = torch.from_numpy(np.load(_runtime_assets_path("hp16384_2d1d_index.npy")))
        """
        # regenerate and consider the bs
        bs = tensor_2d.shape[0]
        return tensor_2d.view(bs, -1)[:, self.indices_2d_to_1d]
    ...
    def forward(self, x):
        if self.config.dev_mode:
            print("feature shape at input: ", x.size())
        features = torch.zeros_like(x[:, :3, :])  # shape = (bs, 3, 131072)
        imgs = torch.split(x[:, :3, :], 16384, dim=2)
        sobels = torch.split(x[:, 3:, :], 16384, dim=2)

        for i in range(8):
            img = imgs[i][:,0,:]  # shape=(bs, 3, 16384)--> (bs, 1, 16384)
            sobel = sobels[i]  # shape=(bs, 1, 16384)
            img_2d = self.torch_1d_to_2d(img.squeeze()).unsqueeze(1)  # torch.Size([bs, 1, 128, 128])  torch.float32
            sobel_2d = self.torch_1d_to_2d(sobel.squeeze()).unsqueeze(1)  # torch.Size([bs, 1, 128, 128])
            img_2d = img_2d.repeat(1, 3, 1, 1)
            # print("img_2d shape: ", img_2d.size())
            # print("sobel_2d shape: ", sobel_2d.size())
            fused_2d = self.edge_attention[i](img_2d, sobel_2d)  # [B, 3, 128, 128]
            fused_1d_1 = self.torch_2d_to_1d(fused_2d[:,0,:].unsqueeze(1)).unsqueeze(1)  # [B, 1, 16384]
            fused_1d_2 = self.torch_2d_to_1d(fused_2d[:,1,:].unsqueeze(1)).unsqueeze(1)
            fused_1d_3 = self.torch_2d_to_1d(fused_2d[:,2,:].unsqueeze(1)).unsqueeze(1)
            fused_1d = torch.cat([fused_1d_1, fused_1d_2, fused_1d_3], dim=1)
            # print("fused_1d shape: ", fused_1d.size())
            features[:, :, i*16384:(i+1)*16384] = fused_1d
...
class SwinHPSobelTransformerSys_2(nn.Module):
    r"""Swin Transformer A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer
        using Shifted Windows` - https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, config: SwinHPTransformerConfig, data_spec: DataSpec, **kwargs):
        ...
        self.edge_attention = nn.ModuleList([
            EdgeAttention_2() for _ in range(8)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(8):
            ...
            # print("img_2d shape: ", img_2d.size())
            # print("sobel_2d shape: ", sobel_2d.size())
            fused_2d = self.edge_attention[i](img_2d, sobel_2d)  # [B, 1, 128, 128]
            fused_1d_1 = self.torch_2d_to_1d(fused_2d[:,0,:].unsqueeze(1)).unsqueeze(1)  # [B, 1, 16384]
            fused_1d = fused_1d_1.repeat(1, 3, 1)
            # print("fused_1d shape: ", fused_1d.size())
            features[:, :, i*16384:(i+1)*16384] = fused_1d
        # print("features shape: ", features.size())
        x = features
...
class SwinHPSobelTransformerSys_3(nn.Module):
    r"""Swin Transformer A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer
        using Shifted Windows` - https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, config: SwinHPTransformerConfig, data_spec: DataSpec, **kwargs):
        ...
        self.edge_attention = nn.ModuleList([
            EdgeAttention_3() for _ in range(8)
        ])
...
class SwinHPSobelTransformerSys_4(nn.Module):
    r"""Swin Transformer A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer
        using Shifted Windows` - https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, config: SwinHPTransformerConfig, data_spec: DataSpec, **kwargs):
        ...
        self.edge_attention = nn.ModuleList([
            EdgeAttention_4() for _ in range(8)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(8):
            ...
            fused_2d = self.edge_attention[i](img_2d, sobel_2d)  # [B, 1, 128, 128]
            fused_1d_1 = self.torch_2d_to_1d(fused_2d[:,0,:].unsqueeze(1)).unsqueeze(1)  # [B, 1, 16384]
            fused_1d_2 = self.torch_2d_to_1d(fused_2d[:,1,:].unsqueeze(1)).unsqueeze(1)  # [B, 1, 16384]
            fused_1d_3 = self.torch_2d_to_1d(fused_2d[:,2,:].unsqueeze(1)).unsqueeze(1)  # [B, 1, 16384]
            fused_1d = torch.cat([fused_1d_1, fused_1d_2, fused_1d_3], dim=1)
            # print("fused_1d shape: ", fused_1d.size())
            features[:, :, i*16384:(i+1)*16384] = fused_1d
        # print("features shape: ", features.size())
...
class SwinHPSobelTransformerSys_5(nn.Module):
    r"""Swin Transformer A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer
        using Shifted Windows` - https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self, config: SwinHPTransformerConfig, data_spec: DataSpec, **kwargs):
        super().__init__()
        data_spec.f_in = data_spec.f_in+1
        self.config = config
        self.data_spec = data_spec
