...
from sce_depth.utils import get_paths, serialize, utils
from sce_depth.training import logging_callbacks as callbacks
from sce_depth.models_lightning import models_lightning

from sce_depth import evaluate
from sce_depth.evaluation import evaluate_config

from sce_depth.data.data import get_data_module

from sce_depth.utils.mlflow_utils import get_tracking_uri
...
def train_model(
    ...
    # If None, will read seed from PL_GLOBAL_SEED env variable or select it randomly.
    ...
    mlf_logger = callbacks.MyMLFlowLogger(
        ...
        experiment_name=train_config.mlflow_expmt,  # "depth_estimation"
    ...
    use_ddp = False
    if pl_config.num_nodes and pl_config.num_nodes > 1:
        use_ddp = True
    if isinstance(pl_config.gpus, int):
        use_ddp = use_ddp or pl_config.gpus > 1
    elif isinstance(pl_config.gpus, (list, tuple)):
        use_ddp = use_ddp or len(pl_config.gpus) > 1
    elif isinstance(pl_config.gpus, str):
        use_ddp = use_ddp or ("," in pl_config.gpus)
    ...
    try:
        ...
        if pl_config.auto_lr_find:
            ...
            print("Fig saved in", str(Path(log_dir.name) / "lr_plot.png"))
...
def main():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="sce_depth/run_configs/depth_estimation/depth_swin_isaac_train_run_config.py",
        help="path to config python file, absolute or relative to HEAL-SWIN path",
    )

    args = parser.parse_args()

    # should return an instance of the dataclass SingleModelTrainRun defined in training/train_config.py
    train_config = utils.get_config_from_config_path(args.config_path, "get_train_run_config")

    # should return an instance of the dataclass PLConfig also defined in training/train_config.py
