...
from sce_depth.training import loss_depth_regression
from sce_depth.evaluation import custom_metrics
...
from sce_depth.data.depth_estimation.data_spec_depth import DepthDataSpec
from sce_depth.models_torch.swin_hp_transformer import SwinHPTransformerConfig, SwinHPTransformerSys
from sce_depth.training.optimizer import OptimizerConfig, get_lightning_optimizer_dict
...
class WoodscapeDepthSwinHP(pl.LightningModule):
    ...
    def __init__(self, config: WoodscapeDepthSwinHPConfig, data_spec: DepthDataSpec, data_config):
        ...
        # self.blur_alpha = nn.Parameter(torch.tensor(config.common_depth_config.blur_alpha))  # 1.0
        self.blur_alpha = config.common_depth_config.blur_alpha # 1.0
        self.loss = loss_depth_regression.get_depth_loss(config.common_depth_config, self.blur_alpha)
    ...
    def shared_step(self, batch, metrics):
        imgs, masks = batch  # imgs.shape = torch.Size([4, 3, 131072]), masks.shape = torch.Size([4, 131072])
        # outputs.shape = torch.Size([4, 1, 131072])
        outputs = self(imgs)
