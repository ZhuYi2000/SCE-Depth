...
@dataclass
class WoodscapeISAACDepthConfig:
    common: WoodscapeCommonConfig = WoodscapeCommonConfig
    common_depth: WoodscapeDepthCommonConfig = WoodscapeDepthCommonConfig
    pred_part: Literal["train", "val"] = "val"  # on which part of the data to predict
    input_nside: int = 128
    input_base_pix: int = 8
    eye_fov: float = 3.0  # field of view of the small eye in the compound eye
    max_depth: float = 10.0  # maximum depth value to use for depth prediction
    dataset_size: int = 600  # size of datasets
    compound_eye: Literal["ico20609", "ico20609+fov3", "ico20609+mean", 'uni10k', 'square100']='ico20609'  # compundeye type
    shuffle_train_val_split: bool = True  # whether to shuffle the data before splitting


@dataclass
class WoodscapeISAACFlatDepthConfig:
    common: WoodscapeCommonConfig = field(default_factory=WoodscapeCommonConfig)
    common_depth: WoodscapeDepthCommonConfig = field(default_factory=WoodscapeDepthCommonConfig)
    pred_part: Literal["train", "val"] = "val"  # on which part of the data to predict
    input_bandwidth: int = 128
    padding: List[int] = field(default_factory=lambda: [0, 0, 0, 0])
    shuffle_train_val_split: bool = True  # whether to shuffle the data before splitting
    nside: int = 128  # nside parameter of HP dataset used for projected predictions
    base_pix: int = 8  # base_pix parameter of HP dataset used for projected predictions


@dataclass
class WoodscapeISAACDepthSobelConfig:
    common: WoodscapeCommonConfig = WoodscapeCommonConfig
    common_depth: WoodscapeDepthCommonConfig = WoodscapeDepthCommonConfig
    pred_part: Literal["train", "val"] = "val"  # on which part of the data to predict
    input_nside: int = 128
    ...
    max_depth: float = 10.0  # maximum depth value to use for depth prediction
    dataset_size: int = 600  # size of datasets
    compound_eye: Literal["ico20609", "ico20609+fov3", "ico20609+mean", 'uni10k', 'square100', 'square3096']='ico20609'  # compundeye type
    shuffle_train_val_split: bool = True  # whether to shuffle the data before splitting


@dataclass
class WoodscapeHPDepthSobelConfig:
    common: WoodscapeCommonConfig = WoodscapeCommonConfig
    common_depth: WoodscapeDepthCommonConfig = WoodscapeDepthCommonConfig
    pred_part: Literal["train", "val"] = "val"  # on which part of the data to predict
    input_nside: int = 128
